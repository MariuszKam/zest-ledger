...
public record Email(String value) {
	...
	private static final Pattern DOMAIN_PATTERN = Pattern.compile("^[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?$");
	...
	private void validateLocalPart(String localPart) {
		...
		for (String part : localParts) {
			if (!LOCAL_PART_PATTERN.matcher(part).matches()) {
				throw new IllegalLocalEmailException("Use of illegal character in local part of email");
			}
		}
	}
	...
	private void validateDomainPart(String domainPart) {
		...
		for (String label : labels) {
			if (!DOMAIN_PATTERN.matcher(label).matches()) {
				throw new IllegalDomainEmailException("Use of illegal character in domain label of email");
			}
		}
	}
}
